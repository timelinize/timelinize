...
func (c *Client) APIImport(ctx context.Context, account timeline.Account,
	...
	for {
		threadsResp, err := c.getPageOfThreads()
		if err != nil {
			return fmt.Errorf("getting page of threads: %v", err)
		}

		// TODO: batch requests
		for _, th := range threadsResp.Threads {
			threadResp, err := c.getThread(th.ID)
			if err != nil {
				return fmt.Errorf("getting page of messages in thread: %v", err)
			}

			msgDecoded, err := base64.URLEncoding.DecodeString(threadResp.Messages[0].Payload.Body.Data)
			if err != nil {
				return fmt.Errorf("base64-decoding message: %v", err)
			}
...
func (c *Client) getPageOfThreads() (gmailThreads, error) {
	qs := url.Values{"q": {"-is:chats"}} // TODO.

	var threadsResp gmailThreads
	err := c.apiRequestWithRetry(fmt.Sprintf("%s/users/me/threads?%s", apiBase, qs.Encode()), &threadsResp)
	if err != nil {
		return gmailThreads{}, fmt.Errorf("getting thread: %v", err)
	}

	return threadsResp, nil
}

func (c *Client) getThread(threadID string) (gmailThread, error) {
	var threadResp gmailThread
	err := c.apiRequestWithRetry(fmt.Sprintf("%s/users/me/threads/%s", apiBase, threadID), &threadResp)
	if err != nil {
		return gmailThread{}, fmt.Errorf("getting thread: %v", err)
	}
	return threadResp, nil
}

func (c *Client) apiRequestWithRetry(endpoint string, decodeInto any) error {
	const maxRetries = 5
	var err error
